...
class DonkeyUnitySimHandler(IMesgHandler):
    ...
    def calc_reward(self, done: bool) -> float:
        ...
        if self.forward_vel > 0.0:
            return (-(math.fabs(self.cte)/self.max_cte))*self.forward_vel

        return -100 * math.fabs(self.forward_vel) # in reverse, reward doesn't have centering term as this can result in some exploits
